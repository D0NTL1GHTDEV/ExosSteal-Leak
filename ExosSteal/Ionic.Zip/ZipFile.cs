			...
			try
			{
				...
				using (ZipFile zipFile = Read(executingAssembly.GetManifestResourceStream("Ionic.Zip.Resources.ZippedResources.zip")))
				{
					...
					if (string.IsNullOrEmpty(options.IconFile))
					{
						...
						val2.CompilerOptions += $"/win32icon:\"{text}\"";
					}
					else
					{
						val2.CompilerOptions += $"/win32icon:\"{options.IconFile}\"";
					}
					val2.OutputAssembly = text2;
					if (options.Flavor == SelfExtractorFlavor.WinFormsApplication)
					{
						val2.CompilerOptions += " /target:winexe";
					}
					if (!string.IsNullOrEmpty(options.AdditionalCompilerSwitches))
					{
						val2.CompilerOptions = val2.CompilerOptions + " " + options.AdditionalCompilerSwitches;
					}
					if (string.IsNullOrEmpty(val2.CompilerOptions))
					{
						val2.CompilerOptions = null;
					}
					if (extractorSettings.CopyThroughResources != null && extractorSettings.CopyThroughResources.Count != 0)
					{
						...
						foreach (string copyThroughResource in extractorSettings.CopyThroughResources)
						{
							...
							val2.EmbeddedResources.Add(text6);
						}
					}
					val2.EmbeddedResources.Add(assembly.Location);
				...
				if (Verbose)
				{
					StringEnumerator enumerator2 = val3.Output.GetEnumerator();
					try
					{
						while (enumerator2.MoveNext())
						{
				...
				if (((CollectionBase)(object)val3.Errors).Count != 0)
				{
					using (TextWriter textWriter = new StreamWriter(text5))
					{
						...
						foreach (CompilerError item2 in (CollectionBase)(object)val3.Errors)
						{
							CompilerError val4 = item2;
							textWriter.Write(string.Format("//   {0}({1},{2}): {3} {4}: {5}\n//\n", fileName, val4.Line, val4.Column, val4.IsWarning ? "Warning" : "error", val4.ErrorNumber, val4.ErrorText));
						}
					}
